...
// Curried function for unit conversion
...
//grabing elements with DOM manipulation 
...
const updateResultsList = () => {
  ...
  units.forEach((unit) => {
    // Skip if current unit matches the selected one
    if (unit !== unitToConvert.value) {
      
      // Convert the ingredient amount
      const finalQuantity = processIngredient(
      ...
      // Create a list item for this conversion
      const li = document.createElement("li");

      // Format result to two decimal places
      li.textContent = `${ingredientName.value}: ${parseFloat(finalQuantity).toFixed(2)} ${unit}`;
...
// Prevent form reload and update results
